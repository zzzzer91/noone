...
static size_t bytes_to_hex(unsigned char *data, size_t len, char *buf) {
    size_t i;
    for (i = 0; i < len; i++) {
        // snprintf() 会在最后加个 '\0'
...
static void test_bytes_to_key() {
    ...
    EXPECT_EQ_STRING("e99a18c428cb38d5f260853678922e0388fc221acae10caf2921f7435051325c", key_hex,
                     key_hex_len);
    free(key_hex);
...
static void test_aes128ctr_encrypt_and_decrypt() {
    ...
    size_t ciphertext_len = encrypt(c_ctx->encrypt_ctx, (uint8_t *)plaintext, plaintext_len,
                                    (uint8_t *)ciphertext);
    ...
    size_t plaintext_len_result = decrypt(c_ctx->decrypt_ctx, (uint8_t *)ciphertext, ciphertext_len,
                                          (uint8_t *)plaintext_result);
    EXPECT_EQ_LONG(plaintext_len, plaintext_len_result);
    EXPECT_EQ_STRING("abc123", plaintext_result, plaintext_len_result);

    // 测试多次加密，一次解密
    ciphertext_len = encrypt(c_ctx->encrypt_ctx, (uint8_t *)plaintext, plaintext_len,
                             (uint8_t *)ciphertext);
    ciphertext_len += encrypt(c_ctx->encrypt_ctx, (uint8_t *)plaintext, plaintext_len,
                              (uint8_t *)ciphertext + ciphertext_len);
    EXPECT_EQ_LONG(plaintext_len * 2, ciphertext_len);
    // 解密
    plaintext_len_result = decrypt(c_ctx->decrypt_ctx, (uint8_t *)ciphertext, ciphertext_len,
                                   (uint8_t *)plaintext_result);
